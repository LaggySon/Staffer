...
export default class helpers {
	...
	calculateDuration(startTime: string, endTime: string) {
		...
		let difference = dayjs(end, 'DD/MM/YYYY HH:mm:ss').diff(dayjs(start, 'DD/MM/YYYY HH:mm:ss'));

		// convert difference from above to a proper dayjsJs duration object
		let duration = dayjs.duration(difference);

		return Math.floor(duration.asHours()) + dayjs.utc(difference).format(':mm');
	}

	buildUrl(event: any, type: any, isCrappyIE: any) {
		let calendarUrl = '';
		...
		switch (type) {
			...
			case 'outlookcom':
				...
				calendarUrl += '&startdt=' + dayjs(event.startTime).format('YYYY-MM-DDTHH:mm:ss');
				calendarUrl += '&enddt=' + dayjs(event.endTime).format('YYYY-MM-DDTHH:mm:ss');
